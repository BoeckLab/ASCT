...
# Aim
# ================================================================
# Assess how antibiotic tolerance (AUCrt0–72h) relates to intrinsic
# growth properties and resistance (MIC) across all ASCT experiments.
#
# This script:
# - Loads the main ASCT phenotype table (MainPhenotypes_ASCTpaper.csv).
# - Extracts per–drug tolerance metrics:
#     AUCrt0–72h / 72 h for 10× and 20× MIC exposures (AMK, FOX, IPM,
#     MXF, TGC, LZD, MIN, AZM early/late).
# - Extracts explanatory variables:
#     • Resistance: log2 MIC per drug (incl. AZMearly / AZMlate),
#     • Growth: Gompertz growth rate (Rate_mean_Gompertz_sd0.3),
#     • Lag: Gompertz lag time (Lag_mean_Gompertz_48h.sd5).
# - Optionally excludes the top 0%, 5% or 10% of isolates with the
#   highest MICs (per drug) to test whether extreme resistance values
#   drive MIC–tolerance correlations.
# - For each drug and exposure (L/H):
#     • Fits linear models of tolerance vs MIC, growth rate, and lag
#       time, extracting Pearson R² and p-values.
#     • Computes Spearman rank correlations and p-values.
# - Builds compact 3-column correlation panels per drug:
#     col 1 = tolerance vs MIC,
#     col 2 = tolerance vs growth rate,
#     col 3 = tolerance vs lag time.
#   AZM panels are split into early-/late-MIC correlations.
# - Annotates each panel with R², Pearson p, Spearman p, n, and the
#   applied MIC-exclusion criterion.
# - Exports all panels as a multi-page PDF:
#     ASCTsupp_Correlation_Tol_v_Growth.Lag.Resistance_Excl.<X>.pdf
#   for integration into the ASCT tolerance analysis.
