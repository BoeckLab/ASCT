...
# ==================================================================
# Script for Single-Cell Tracking, PI-Intensity Cleanup and
# Live-Cell Fraction / Ranked-Trajectory Visualisation
# ==================================================================
# This R script processes Trkv2 single-cell tracking outputs and
# ilastik object classification (POC) results to generate live-cell
# fractions and ranked single-cell PI–intensity trajectories for
# ASCT experiments.
#
# Functionality:
# - Defines working directories and detects available tracking and
#   BaSic-corrected POC outputs (old vs. new background correction).
# - Loads per-object tracking data and merges it with ilastik
#   classification tables (Mean.Intensity, Predicted.Class).
# - Bridges missing frames by propagating previous PI intensity and
#   class labels (up to a defined limit) and filters tracks with too
#   many gaps.
# - Applies the “twice positive always positive” ilastik definition
#   (Def.2) to derive stable piPOS / piNEG calls per track.
# - Computes per–well live-cell fraction (LCF) over time from piNEG
#   frequencies and aligns these with time metadata from population
#   analysis.
# - Creates unique track IDs, ranks tracks by cumulative piPOS labels,
#   and generates:
#     * ranked trajectory plots (linear and log10 PI colour scales),
#     * dual-axis plots overlaying LCF curves and ranked trajectories,
#     * a manuscript-style figure (Fig. 1c) combining survival curve
#       and coloured single-cell tracks.
#
# Usage:
# - Run from an ASCT experiment root containing:
#   * `Tracking_LC`, `*_Tracking_labeloid/`, and `*_OCsimple.pocbSd/`
#     directories.
# - Ensure Trkv2 tracking CSVs and ilastik POC CSVs are present in
#   the expected folder structure.
# - Adjust file paths / well selection in the final plotting section
#   when applying this script to other experiments or figures.
...
perWell.trk.pi.df  %>%
  ...
  theme( legend.position = "right",
       #  guides(color = guide_colorbar(title = "Log mean Pi intensity")),
...
CHECK <- perWell.trk.pi.df  %>%
  #filter(UniqueTrackID <= 500)%>%
  ungroup()%>%
#  mutate(Ila.Mean.Intensity = if_else(Ila.Mean.Intensity == 0.000000000, 1, Ila.Mean.Intensity))%>%
  mutate(Ila.Mean.Intensity = if_else(Ila.Mean.Intensity <= 1, 1, Ila.Mean.Intensity))%>%
...
plot.path <- paste(
  genDir, "/",
  unique(perWell.trk.pi.df$ExpFile), "_",
  unique(perWell.trk.pi.df$Well_coordinate),
  "_Results/",
  unique(perWell.trk.pi.df$ExpFile), "_",
  unique(perWell.trk.pi.df$Well_coordinate),
...
# Join the time-kill curve values to your single-cell dataframe
# Join the time-kill curve values to your single-cell dataframe
perWell.trk.pi.df <- perWell.trk.pi.df %>%
  left_join(
    Raw.TKC.df %>%
      dplyr::select(Time_Hrs, Well_coordinate, LCF) %>%
      dplyr::rename(LCF_join = LCF),
    by = c("Time_Hrs", "Well_coordinate")
  ) %>%
  mutate(curve_y = LCF_join * Norm.factor,
         below_curve = Rank <= curve_y)
...
perWell.trk.pi.df %>%
  ...
  # plot the tracks, dim those below the survival curve
  geom_line(aes(alpha = below_curve)) +
  ...
  scale_colour_gradientn(
    name = "Mean PI intensity\nper object (a.u.)",
    colors = c("black", "black","red"),
    values = scales::rescale(c(0, 1.8, 3)),
    limits = c(0, 3),
    space = "Lab",
    breaks = c(0, 2),
    labels = c("Low", "High")
  ...
  # labels
  labs(
    x = "Antibiotic exposure (hours)\n Moxifloxacin 20x MIC",
    y = paste0(
      "Fraction bacteria alive (%)\n(",
      Norm.factor, " single cell trajectories)"
    )
  ) +
  
  # x-axis ticks only at 0, 24, 48, 72
  scale_x_continuous(breaks = c(0, 24, 48, 72)) +
  
  # convert y-axis values from Rank to percent
  scale_y_continuous(labels = function(x) round(x * 100 / Norm.factor)) +
...
perWell.trk.pi.df %>%
  ...
  scale_colour_gradientn(
    name = "Mean PI intensity\nper object (a.u.)",
    colors = c("black", "black", "red"),
    values = scales::rescale(c(0, 1.9, 3.0951)),
    limits = c(0,3.0951),
    space = "Lab",
    breaks = c(0,3.0951),
    labels = c("Low", "High")
  ...
  labs(
    x = "Antibiotic exposure (hours)\nMoxifloxacin 20x MIC",
    y = paste0(
      "Percent bacteria alive (%)\n(",
      Norm.factor, " single cell trajectories)"
    )
  ) +
  annotate(
    "text",
    x = 24,                      # horizontal midpoint between 0 and 48
    y = 0.2 * Norm.factor,       # 20% height of your Rank axis
    label = "Area under\n the time-kill curve",
    color = "white",
    size = 5,
    fontface = "bold"
  ...
  # Left axis: percentage ticks 0,20,...100 ; Right axis: TrackIDs
  scale_y_continuous(
    limits = c(0, Norm.factor),
    breaks = seq(0, Norm.factor, by = Norm.factor / 5),
    labels = function(x) round(x * 100 / Norm.factor),
    sec.axis = sec_axis(
      ~ ., 
      name = "TrackIDs"
    )
  ...
  theme(
    ...
    # remove right (secondary) axis completely
    axis.line.y.right = element_blank(),
