...
# Aim
# ================================================================
# Visualise the distribution of MICs across M. abscessus isolates and
# relate MIC values to clinical treatment outcomes.
#
# This script:
# - Loads MIC values and ATS clinical outcome categories (cleared vs
#   persistent vs unknown) for all isolates and antibiotics.
# - Recodes outcome labels, counts isolates per MIC, and computes, for
#   each antibiotic and outcome class, the percentage of isolates at
#   each MIC.
# - Fills gaps in MIC dilution series (per antibiotic / outcome) so that
#   missing concentrations appear as zero-count bins.
# - Converts MICs from log2 scale to µg/ml and harmonises dynamic ranges
#   using antibiotic-specific MIC ranges from an external metadata table.
# - Constructs dual-axis faceted histograms:
#     • primary y-axis: number of isolates at each MIC (bar height),
#     • secondary y-axis: percentage of isolates per outcome category at
#       each MIC (overlaid lines and points).
# - Generates alternative views with:
#     • full MIC dynamic range per antibiotic,
#     • reduced / free MIC ranges,
#     • exclusion of BDQ and RFB to focus on remaining antibiotics.
# - Exports the resulting figures (per-antibiotic MIC histograms with
#   outcome overlays) as multi-panel PDFs for inclusion in ASCT kill
#   curve analyses.
...
LB % of outcom should reflect the trend of the bars similar like a histogram. Adjust calculation to be per abx per outcome.
...
```{r}
 ...
 plot.path <- paste(resDir,
                     "/",
  "ASCTfig_MIC_Alldata_MICbarplot_Outcome_curves_MICµgml_subMICrange.pdf",
  sep="")
...
#Bob <-full_df %>%
  ...
  geom_bar(aes(y = N.per.MIC),
           ...
           position = "dodge2",
           alpha = 0.75) +
  geom_line( data = MICµg.gg_MIC.perc.outcome.Update.full_TEST,
             aes(y = Percent_perMIC * scaling.factor,
               color = ClearedVSpersistant_ATS,
                group = ClearedVSpersistant_ATS), 
            alpha =0.5, 
            size = 0.2) +
  geom_point(data = MICµg.gg_MIC.perc.outcome.Update.full_TEST,
             aes(y = Percent_perMIC * scaling.factor,
               color = ClearedVSpersistant_ATS,
            group = ClearedVSpersistant_ATS),
             alpha =0.8, 
 ...
 scale_fill_manual(values = c("Cleared" = "navyblue",
  ...
  theme(axis.line.y.right = element_line(color = "black"), 
        ...
        legend.position = "top",           # Position the legend at the top
         ...
         axis.title = element_text(size = 14, face = "plain"),  # Axis titles not bold
    axis.text = element_text(size = 12, face = "plain"),   # Axis text not bold
    legend.title = element_text(size = 12, face = "plain"),# Legend title not bold
    legend.text = element_text(size = 10, face = "plain"), # Legend text not bold
    plot.title = element_text(size = 16, face = "plain") ,  # Plot title not bold
    
        axis.title.y.right = element_text(color = "black")) +
  facet_wrap(~Abx.con, scales = "free_x")
dev.off()
...
```{r}
 plot.path <- paste(resDir,
                     "/",
  "ASCTfig_MIC_Alldata_MICbarplot_Outcome_curves_MICµgml_subMICrange-FreeScale.pdf",
  sep="")
