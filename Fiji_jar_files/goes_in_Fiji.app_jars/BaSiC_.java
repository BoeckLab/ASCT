...
//	ImagePlus getDarkfield();
//	void setFlatfield();


//public class TestPlugin_ implements PlugInFilter{
public class BaSiC_ implements PlugIn, DialogListener {
    /*
    @modifications by Loïc Sauteur:
        - added package line at the beginning, due to maven build project in IDE
            -> commented out
        - added global private varialbe, which is the corrected_imp
        - added getter for corrected_imp
            this way one can get the image via scripting (also headless mode)
        - commented out the "IJ.run("Enhance Contrast"..." calls (3 times)
            - this comment is useless, since it acts on the opened/displayed image
            - and prevents working in headless mode
        - the exec() function does not show any results anymore,
        instead the results are saved to global variables, which are shown after the exec() was called
        see at "@modification by LS" tags 
     */
    ...
    private ImagePlus corrected_imp; // @modification by LS
    private Plot basefluor_plot;
    ...
    // getters by LS
    public ImagePlus getCorrected_imp() { // @modification by LS
        return corrected_imp;
    }
    public Plot getBasefluor_plot() {
        return basefluor_plot;
    }



    private static class Shading { // made static @modification by LS
...
//	private static String[] colors = {"Input_sequence","Precomputed_flatfield","Precomputed_darkfield"};
    ...
    void processDialog(GenericDialog gd, int[] wList) {
        ...
        if (StackIndex == wList.length) {
            ...
            //* execute the command
            Shading outputShading = exec(imp, imp_flat, imp_dark, myShadingEstimationChoice, myShadingModelChoice, myParameterChoice, lambda_flat, lambda_dark, myDriftChoice, myCorrectionChoice);
            // show the outputs now instead of inside the exec() function // @modification by LS
            corrected_imp.show();
            if (!myDriftChoice.equals(driftOptions[0])) basefluor_plot.show();
            outputShading.flatfield.show();
            if (myShadingModelChoice.equals(shadingModelOptions[1])) outputShading.darkfield.show();
...
/*public void run(String params, ImagePlus imp)
{
    ...
    public Shading exec(ImagePlus imp, ImagePlus imp_flat, ImagePlus imp_dark, String myShadingEstimationChoice, String myShadingModelChoice, String myParameterChoice, double lambda_flat, double lambda_dark, String myDriftChoice, String myCorrectionChoice) {
        stack = imp.getStack();
...
//        outputShading.flatfield.show(); // @modification by LS
...
//            if (!headlessmode) outputShading.darkfield.show(); // @modification by LS
        }
...
//	    	    	temporal_rt.addValue("Ratioflat",ratioflat[j-1]);
            ...
            basefluor_plot = new Plot("Basefluor", "FrameNo", "Basefluor", x, basefluor);
//            basefluor_plot.show(); // @modification by LS
//	    	if (myOptions.varyingflatfieldOpt)
//	    	{
...
//		    		imageCorrected = imageCorrected.convertToFloat();
            ...
            corrected_imp = new ImagePlus(str1.concat(title), corrected_stack);
            ce.stretchHistogram(corrected_imp.getProcessor(), 0.01);
//            IJ.run("Enhance Contrast", "saturated = 0.01");
//            corrected_imp.show(); // @modification by LS
        }

        return outputShading; // @modification by LS
    }
